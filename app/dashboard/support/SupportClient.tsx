...
export default function SupportClient({ isYearly }: { isYearly: boolean }) {
    ...
    const supportEmail = 'support@akidsy.com';
    const emailSubject = isYearly ? '?subject=[PRIORITY]%20Support%20Request' : '?subject=Support%20Request';
    const emailLink = `mailto:${supportEmail}${emailSubject}`;
    ...
    return (
        <div className="max-w-4xl mx-auto py-8">
            ...
            {isYearly && (
                <div className="max-w-2xl mx-auto mb-12 bg-sunshine/20 border-4 border-sunshine rounded-2xl p-4 flex items-center justify-center gap-3 shadow-[4px_4px_0_0_#F9E104]">
                    <CheckCircle2 className="w-6 h-6 text-persimmon flex-shrink-0" />
                    <p className="font-bold text-navy text-lg text-center">
                        Priority Support Active: Our team will get back to you within 4 hours.
                    </p>
                </div>
            )}
